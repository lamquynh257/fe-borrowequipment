...
const BorrowItem = ({ params }) => {
  ...
  const handleConfirm = async () => {
    ...
    // console.log(endTime._d);
    // Tính số ngày và số giờ
    let days = Math.floor(duration.asDays());
    let hours = duration.hours();
    const minutes = duration.minutes();

    // Nếu số giờ vượt quá 24, chuyển đổi thành số ngày
    if (hours >= 24) {
      days += Math.floor(hours / 24);
      hours = hours % 24;
    }

    // console.log(`Số ngày sử dụng: ${days} ngày, ${hours} giờ ${minutes} phút`);

    const time1 = dataConfirm.equipmentinfo?.usetime; // thời gian sử dụng trước đó
    const time2 = `${days} ngày, ${hours} giờ ${minutes} phút`; // thời gian sử dụng case hiện tại
    console.log(`${days} ngày, ${hours} giờ ${minutes} phút`);

    const totalTimeUse = addTimeValues(time1, time2);
    axios
      .post(
        `${process.env.NEXT_PUBLIC_BACKEND_API}/api/confirmborrow`,
        {
          id: dataConfirm.id, // id của borrow
          end: endTime._d, // gửi lên để set endtime cho borrow
  ...
  const newAllEquip = allEquip.filter(
    (item) =>
      item.branchname === parseInt(params.id) && item.status === "Sẵn sàng"
  );

  const newData = data.filter(
    (item) =>
      item.equipmentinfo.branchinfo.id === parseInt(params.id) &&
      item.status === "Đã được trả"
  );
  // console.log(newData);
